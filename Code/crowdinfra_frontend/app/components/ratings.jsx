...
export const InfiniteMovingCards = ({ items, direction = "left", speed = "normal" }) => {
    ...
    return (
        <div className="scroller-container relative max-w-7xl overflow-hidden">
            <ul
                ...
                style={{
                    animation: `scrollX ${speed === "fast" ? "20s" : speed === "normal" ? "40s" : "80s"} linear infinite`,
                    animationDirection: direction === "left" ? "normal" : "reverse",
                }}
            >
                {[...items, ...items].map((item, idx) => ( // Duplicate items for seamless looping
                    <li
                        key={idx}
...
const CustomerReviewCarousel = () => {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const response = await axios.get(`http://localhost:5030/api/rating/reviews`);
