import Link from 'next/link';

const Footer = () => {
    return (
        <footer className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white py-8 shadow-2xl">
            <div className="container mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 place-items-start">
                    {/* Company Info */}
                    <div className="w-full space-y-3 bg-gray-800/30 p-5 rounded-xl transform transition-all duration-500 hover:bg-gray-800/50 hover:scale-105">
                        <div>
                            <h3 className="text-3xl font-bold mb-4 text-white bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-600 flex items-center">
                                CrowdInfra
                            </h3>
                            <p className="text-gray-300 text-opacity-80 leading-relaxed">
                                Building better infrastructure through community collaboration and innovative solutions.
                            </p>
                        </div>
                        
                        
                        <div className="flex space-x-4">
                            {[
                                { 
                                    href: "https://twitter.com", 
                                    icon: (
                                        <svg className="h-7 w-7 transition-all duration-700 group-hover:rotate-360 group-hover:scale-125" fill="url(#twitterGradient)" viewBox="0 0 24 24">
                                            <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                                        </svg>
                                    ),
                                    gradient: (
                                        <linearGradient id="twitterGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" stopColor="#1DA1F2" />
                                            <stop offset="100%" stopColor="#14538B" />
                                        </linearGradient>
                                    )
                                },
                                { 
                                    href: "https://linkedin.com", 
                                    icon: (
                                        <svg className="h-7 w-7 transition-all duration-700 group-hover:rotate-360 group-hover:scale-125" fill="url(#linkedinGradient)" viewBox="0 0 24 24">
                                            <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                                        </svg>
                                    ),
                                    gradient: (
                                        <linearGradient id="linkedinGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" stopColor="#0A66C2" />
                                            <stop offset="100%" stopColor="#004182" />
                                        </linearGradient>
                                    )
                                },
                                { 
                                    href: "https://github.com", 
                                    icon: (
                                        <svg className="h-7 w-7 transition-all duration-700 group-hover:rotate-360 group-hover:scale-125" fill="url(#githubGradient)" viewBox="0 0 24 24">
                                            <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
                                        </svg>
                                    ),
                                    gradient: (
                                        <linearGradient id="githubGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" stopColor="#333" />
                                            <stop offset="100%" stopColor="#000" />
                                        </linearGradient>
                                    )
                                },
                                { 
                                    href: "https://instagram.com", 
                                    icon: (
                                        <svg className="h-7 w-7 transition-all duration-700 group-hover:rotate-360 group-hover:scale-125" fill="url(#instagramGradient)" viewBox="0 0 24 24">
                                            <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.148 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.148-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.948-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.441 1.441 1.441c.795 0 1.439-.646 1.439-1.441s-.644-1.44-1.439-1.44z" />
                                        </svg>
                                    ),
                                    gradient: (
                                        <linearGradient id="instagramGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" stopColor="#F58529" />
                                            <stop offset="50%" stopColor="#DD2A7B" />
                                            <stop offset="100%" stopColor="#515BD4" />
                                        </linearGradient>
                                    )
                                }
                            ].map(({ href, icon, gradient }) => (
                                <a 
                                    key={href} 
                                    href={href} 
                                    className="group relative flex items-center justify-center transform transition-all duration-300 hover:scale-125"
                                >
                                    <svg width="0" height="0" className="absolute">
                                        {gradient}
                                    </svg>
                                    <span className="sr-only">{href.split('//')[1].split('.')[0]}</span>
                                    {icon}
                                </a>
                            ))}
                        </div>
                    </div>
                    
                    {/* Quick Links */}
                    <div className="w-full space-y-4 bg-gray-800/30 p-5 rounded-xl transform transition-all duration-500 hover:bg-gray-800/50 hover:scale-105">
                        <h3 className="text-2xl font-bold mb-4 text-white bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-blue-500">
                            Quick Links
                        </h3>
                        <ul className="space-y-2">
                            {[
                                { href: "/home", label: "Home" },
                                { href: "/search-demands", label: "Demands" },
                                { href: "/properties", label: "Properties" },
                                { href: "/Post Demand", label: "Post Demand" },
                                { href: "/contact", label: "Contact" }
                            ].map(({ href, label }) => (
                                <li key={href}>
                                    <Link 
                                        href={href} 
                                        className="text-gray-300 hover:text-white hover:pl-3 transition-all duration-300 inline-block border-l-2 border-transparent hover:border-blue-500"
                                    >
                                        {label}
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    </div>
                    
                    {/* Contact Info */}
                    <div className="w-full space-y-4 bg-gray-800/30 p-5 rounded-xl transform transition-all duration-500 hover:bg-gray-800/50 hover:scale-105">
                        <h3 className="text-2xl font-bold mb-4 text-white bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600">
                            Contact Us
                        </h3>
                        <address className="not-italic text-gray-300 space-y-2">
                            <p className="flex items-center space-x-2 hover:text-white transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                                <span>123 Infrastructure Road</span>
                            </p>
                            <p>Building City, BC 10000</p>
                            <p>
                                Email: <a 
                                    href="mailto:info@crowdinfra.com" 
                                    className="hover:text-blue-400 transition-colors underline"
                                >
                                    info@crowdinfra.com
                                </a>
                            </p>
                            <p>
                                Phone: <a 
                                    href="tel:+1234567890" 
                                    className="hover:text-green-400 transition-colors underline"
                                >
                                    (123) 456-7890
                                </a>
                            </p>
                        </address>
                    </div>
                </div>
                
                {/* Copyright */}
                <div className="border-t border-gray-700/50 mt-8 pt-6 text-center">
                    <p className="text-gray-400 hover:text-white transition-colors animate-pulse bg-clip-text text-transparent bg-gradient-to-r from-gray-400 to-gray-600">
                        &copy; {new Date().getFullYear()} CrowdInfra. All rights reserved.
                    </p>
                </div>
            </div>
        </footer>
    );
};

export default Footer;