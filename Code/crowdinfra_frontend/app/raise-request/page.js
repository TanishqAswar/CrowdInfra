...
const RaiseRequestPage = () => {
  ...
  const handleAddComment = (requestId) => {
    if (newComment.trim()) {
      ...
      setComments({
        ...comments,
        [requestId]: [...(comments[requestId] || []), comment]
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 py-10 text-gray-100">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="max-w-3xl mx-auto mt-8 bg-white/10 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-white/20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <label className="block text-lg font-medium text-gray-200 mb-2">Request Title</label>
                <input
                  type="text"
        ...
        {/* Request List */}
        <div className="mt-12">
          <h2 className="text-3xl font-bold mb-6 text-center text-gray-200">Community Requests</h2>
          ...
          {requests.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {requests.map(request => (
                <div key={request.id} className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 shadow-xl">
                  <h3 className="text-xl font-bold text-blue-400 mb-2">{request.title}</h3>
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button 
                      onClick={() => setActiveRequest(activeRequest === request.id ? null : request.id)}
                      className="flex items-center space-x-1 text-gray-300 hover:text-blue-400 transition-colors"
                    >
